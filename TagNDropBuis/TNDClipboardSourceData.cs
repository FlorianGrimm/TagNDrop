...
namespace TagNDropBuis {
    ...
    /// </summary>
    public class TNDClipboardSourceData : System.Windows.IDataObject {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TNDClipboardSourceData"/> class.
        /// </summary>
        /// <param name="underlyingDataObject">The underlying data object to wrap.</param>
        public TNDClipboardSourceData(System.Windows.IDataObject underlyingDataObject) {
        ...
        #region ReadFileGroupDescriptor
        public static string[] ReadFileGroupDescriptorA(MemoryStream ms) {
            ...
            try {
                ...
                var fileGroupDescriptor = Marshal.PtrToStructure<NativeMethods.FILEGROUPDESCRIPTORA>(fileGroupDescriptorWPointer);
                ...
                for (int fileDescriptorIndex = 0; fileDescriptorIndex < cItems; fileDescriptorIndex++) {
                    ...
                    NativeMethods.FILEDESCRIPTORA fileDescriptor = Marshal.PtrToStructure<NativeMethods.FILEDESCRIPTORA>(fileDescriptorPointer);
        ...
        public string GetFormatDescription() {
            if (this.underlyingDataObject == null) { return null; }
        ...
        public object GetData(string format, bool autoConvert) {
            ...
            switch (format) {
                case "FileGroupDescriptor": {
                        using (MemoryStream fileGroupDescriptorStream = this.underlyingDataObject.GetData("FileGroupDescriptor", autoConvert) as MemoryStream) {
                            return ReadFileGroupDescriptorA(fileGroupDescriptorStream);
                        }
                    }
                case "FileGroupDescriptorW": {
                        using (MemoryStream fileGroupDescriptorStream = this.underlyingDataObject.GetData("FileGroupDescriptorW", autoConvert) as MemoryStream) {
                            return ReadFileGroupDescriptorW(fileGroupDescriptorStream);
                        }
                    }
                case "FileContents": {
                        return this.ReadFileContents(autoConvert);
                    }
                default: {
        ...
        public List<TNDDropItem> ReadFileContents(bool autoConvert) {
            // override the default handling of FileContents which returns the
            // contents of the first file as a memory stream and instead return
            // a array of MemoryStreams containing the data to each file dropped

            // get the array of filenames which lets us know how many file contents exist
            string[] fileContentNames = null;
            using (MemoryStream fileGroupDescriptorStream = this.underlyingDataObject.GetData("FileGroupDescriptorW", autoConvert) as MemoryStream) {
                fileContentNames = ReadFileGroupDescriptorW(fileGroupDescriptorStream);
            }
            var length = fileContentNames.Length;
            var result = new List<TNDDropItem>(length);
            ...
            for (int fileIndex = 0; fileIndex < length; fileIndex++) {
                ...
                var fileName = fileContentNames[fileIndex];
                try {
                    var ms = this.GetData("FileContents", fileIndex);
                    var displayName = System.IO.Path.GetFileName(fileName);
                    result.Add(new TNDDropItem() { Name = fileName, DisplayName = displayName, Content = ms.ToArray() });
                } catch (Exception exception) {
                    result.Add(new TNDDropItem() { Name = fileName, DisplayName = fileName, LastException = exception });
                }
            }
            return result;
        }

        public IEnumerable<TNDDropItem> ReadFileDrop() {
            var fileNames = this.underlyingDataObject.GetData("FileDrop") as string[];
            if (fileNames == null) { return null; }
            var result = new List<TNDDropItem>(fileNames.Length);
            foreach (var fileName in fileNames) {
                try {
                    var displayName = System.IO.Path.GetFileName(fileName);
                    var content = System.IO.File.ReadAllBytes(fileName);
                    result.Add(new TNDDropItem() { Name = fileName, DisplayName = displayName, Content = content });
                } catch (Exception exception) {
                    result.Add(new TNDDropItem() { Name = fileName, DisplayName = fileName, LastException = exception });
                }
            }
